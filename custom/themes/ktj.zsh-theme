...
prompt_context() {
  ...
  prompt_segment NONE NONE "%(%{%F{red}%})%{$fg_bold[magenta]%}$username%{$fg[white]%}@%{$fg_bold[green]%}${hostname}%{$reset_color%}:"
...
omz_git_color() {
  ...
  if [[ ! $git_status =~ "working directory clean" && ! $git_status =~ "working tree clean"  ]]; then
    prompt_segment red white%B
  elif [[ $git_status =~ "Your branch is ahead of" ]]; then
    prompt_segment yellow%B black
  elif [[ $git_status =~ "have diverged" ]]; then
    prompt_segment 054 white%B
  elif [[ $git_status =~ "nothing to commit" ]]; then
    prompt_segment green black%B
  else
